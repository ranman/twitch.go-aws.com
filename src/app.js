...
const APIBaseURL = "https://1w7f57jh5d.execute-api.us-east-1.amazonaws.com/prod/streams/"
...
const hostpotIteratorURL = new URL(APIBaseURL + hotspotStream + "/sharditerator")
...
async function getShardIterator(shard) {
    const response = await fetch(hostpotIteratorURL + "?shard-id=" + encodeURIComponent(shard))
    const json = await response.json()
...
setInterval(async () => {
    shards.forEach(async (value, key, map) => {
        ...
        hotspotsData.Records.forEach(async record => {
            const hotspots = JSON.parse(JSON.parse(atob(record.Data)).HOTSPOTS_RESULT).hotspots
